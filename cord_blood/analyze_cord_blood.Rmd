...
```
So their is a fair bit of skew in this data past the apparent double peak at the lower end of the distribution. Let's take a closer look at the lower end of the distribution:
...
* `r length(unique(cord_blood_kinases$ensembl_gene_id))` unique kinases are represented in the data
* `r round(100*sum(cord_blood_kinases$counts)/sum(cord_blood_filt$counts),2)`% of the mapped reads are to a kinase
* `r round(100*length(unique(cord_blood_kinases$barcode))/length(unique(cord_blood_filt$barcode)))`% of the bone marrow cells express at least one kinase
* `r round(100*length(unique(cord_blood_dark_kinases$barcode))/length(unique(cord_blood_filt$barcode)))`% of the cells express a dark kinase


```{r sanity_checking, eval=FALSE, echo=FALSE}
ggplot(cord_blood_filt_per_cell_summary,aes(x=unique_genes,y=read_count)) + 
  geom_hex() + theme_berginski() +
  labs(x='Unique Gene Count',y="Read Count Sum")
```
